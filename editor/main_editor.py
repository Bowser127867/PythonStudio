...
PROJECT_PACKAGE_MAGIC = b"PYSGAME1"
PROJECT_PACKAGE_ITERATIONS = 0
PROJECT_PACKAGE_KEY = b"PythonStudio project package key v1"
...
class PythonStudioEditor(tk.Tk):
    def __init__(self):
        ...
        self.after(100, self.poll_thread_results)
    ...
    def _build_menu(self) -> None:
        ...
        file_menu.add_command(label="Import Encrypted Project...", command=self.import_encrypted_project_dialog)
        file_menu.add_command(label="Export Encrypted Project...", command=self.export_encrypted_project_dialog)
    ...
    def export_encrypted_project_dialog(self) -> None:
        self.save_scene()
        ...
        if not (project_path / "scene.json").exists():
            messagebox.showerror(
                "Export Project Error",
                "Select the game project folder that contains scene.json.\n\n"
                "To save into Documents, first select the project folder, then choose Documents in the save dialog."
            )
            self.log(f"Export cancelled: {project_path} does not contain scene.json.")
            return

        output_path = filedialog.asksaveasfilename(
            title="Export Encrypted Project",
            initialdir=project_path.parent,
            initialfile=f"{project_path.name}.pysg",
            defaultextension=".pysg",
            filetypes=[
                ("PythonStudio Game Package", "*.pysg *.pysgame"),
                ("All files", "*.*"),
            ],
        )
        if not output_path:
        ...
        if output_path.suffix.lower() not in (".pysg", ".pysgame"):
            output_path = output_path.with_suffix(".pysg")

        self.log(f"Exporting encrypted project: {project_path} -> {output_path}")
        threading.Thread(
            target=self.export_encrypted_project_worker,
            args=(project_path, output_path),
            daemon=True,
        ).start()

    def import_encrypted_project_dialog(self) -> None:
        package_path = filedialog.askopenfilename(
            title="Import Encrypted Project",
            initialdir=PROJECTS_DIR,
            filetypes=[
                ("PythonStudio Game Package", "*.pysg *.pysgame"),
                ("All files", "*.*"),
        ...
        if self.package_iterations(package_data) > 0:
            legacy_password = simpledialog.askstring(
                "Legacy Project Package",
                "This package was made with the old password system.\nEnter its old password to import it:",
                show="*",
            )
            if not legacy_password:
                return

        self.log(f"Importing encrypted project: {package_path}")
        threading.Thread(
            target=self.import_encrypted_project_worker,
            args=(package_path, package_data, legacy_password),
            daemon=True,
        ).start()

    def export_encrypted_project_worker(self, project_path: Path, output_path: Path) -> None:
        try:
            zip_bytes = self.zip_project_folder(project_path, output_path)
            encrypted_bytes = self.encrypt_project_bytes(zip_bytes)
            self.write_project_package(output_path, encrypted_bytes)
            self.validate_project_package(output_path)
            self.thread_results.put(("export_project", output_path, None))
        except Exception as error:
            self.thread_results.put(("export_project", output_path, error))

    def write_project_package(self, output_path: Path, encrypted_bytes: bytes) -> None:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        temp_path = output_path.with_name(f".{output_path.name}.tmp")
        try:
            temp_path.write_bytes(encrypted_bytes)
            if not temp_path.exists() or temp_path.stat().st_size != len(encrypted_bytes):
                raise OSError(f"Package write failed: {temp_path}")
            temp_path.replace(output_path)
            if not output_path.exists() or output_path.stat().st_size == 0:
                raise OSError(f"Package was not created: {output_path}")
        finally:
            if temp_path.exists():
                try:
                    temp_path.unlink()
    ...
    def validate_project_package(self, package_path: Path) -> None:
        package_data = package_path.read_bytes()
        if self.package_iterations(package_data) != PROJECT_PACKAGE_ITERATIONS:
            raise ValueError(f"Package was written with an unexpected format: {package_path}")

        zip_bytes = self.decrypt_project_bytes(package_data)
        if not zipfile.is_zipfile(io.BytesIO(zip_bytes)):
            raise ValueError(f"Package validation failed: {package_path}")

    def finish_project_export(self, output_path: Path, error: Optional[Exception]) -> None:
        if error is not None:
            messagebox.showerror("Project Export Error", str(error))
            self.log(f"Failed to export encrypted project: {error}")
            return

        size = output_path.stat().st_size if output_path.exists() else 0
        self.log(f"Exported encrypted project: {output_path} ({size} bytes, no-password format)")
        messagebox.showinfo("Project Exported", f"Encrypted project saved:\n{output_path}")

    def import_encrypted_project_worker(
        self,
        package_path: Path,
        package_data: bytes,
        ...
        try:
            zip_bytes = self.decrypt_project_bytes(package_data, legacy_password)
            extracted_root = self.extract_project_zip(zip_bytes, PROJECTS_DIR)
            self.thread_results.put(("import_project", extracted_root, None))
        except Exception as error:
            self.thread_results.put(("import_project", None, error))

    def finish_project_import(self, extracted_root: Optional[Path], error: Optional[Exception]) -> None:
        if error is not None or extracted_root is None:
            messagebox.showerror("Project Import Error", str(error))
            self.log(f"Failed to import encrypted project: {error}")
            return

        self.log(f"Imported encrypted project: {extracted_root}")
        messagebox.showinfo("Project Imported", f"Project imported:\n{extracted_root}")

        scene_path = self.find_imported_scene(extracted_root)
        if scene_path and messagebox.askyesno("Open Imported Scene", f"Open scene now?\n{scene_path}"):
            self.load_scene(scene_path)

    def zip_project_folder(self, project_path: Path, output_path: Path) -> bytes:
        ...
        with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
            for path in sorted(project_path.rglob("*")):
                ...
                if path.resolve() == output_resolved:
                    continue
                archive.write(path, Path(project_path.name) / path.relative_to(project_path))
        return buffer.getvalue()

    def encrypt_project_bytes(self, data: bytes) -> bytes:
        salt = secrets.token_bytes(16)
        nonce = secrets.token_bytes(16)
        key = self.package_key_bytes(salt, PROJECT_PACKAGE_ITERATIONS)
        encryption_key = key[:32]
        mac_key = key[32:]
        encrypted_data = self.xor_with_keystream(data, encryption_key, nonce)
        header = (
            PROJECT_PACKAGE_MAGIC
            + PROJECT_PACKAGE_ITERATIONS.to_bytes(4, "big")
            + salt
            + nonce
        )
        tag = hmac.new(mac_key, header + encrypted_data, hashlib.sha256).digest()
        return header + tag + encrypted_data

    def decrypt_project_bytes(self, package_data: bytes, legacy_password: Optional[str] = None) -> bytes:
        minimum_size = len(PROJECT_PACKAGE_MAGIC) + 4 + 16 + 16 + 32
        if len(package_data) < minimum_size:
            raise ValueError("Package file is too small.")

        offset = 0
        magic = package_data[offset:offset + len(PROJECT_PACKAGE_MAGIC)]
        offset += len(PROJECT_PACKAGE_MAGIC)
        if magic != PROJECT_PACKAGE_MAGIC:
            raise ValueError("This is not a valid PythonStudio package.")

        iterations = int.from_bytes(package_data[offset:offset + 4], "big")
        offset += 4
        salt = package_data[offset:offset + 16]
        offset += 16
        nonce = package_data[offset:offset + 16]
        offset += 16
        saved_tag = package_data[offset:offset + 32]
        offset += 32
        encrypted_data = package_data[offset:]
        header = package_data[:len(PROJECT_PACKAGE_MAGIC) + 4 + 16 + 16]

        key = self.package_key_bytes(salt, iterations)
        encryption_key = key[:32]
        mac_key = key[32:]
        expected_tag = hmac.new(mac_key, header + encrypted_data, hashlib.sha256).digest()
        if not hmac.compare_digest(saved_tag, expected_tag):
            if legacy_password is None:
                raise ValueError("Package is corrupted or was created with the old password system.")

            key = self.legacy_password_key_bytes(legacy_password, salt, iterations)
            encryption_key = key[:32]
            mac_key = key[32:]
            expected_tag = hmac.new(mac_key, header + encrypted_data, hashlib.sha256).digest()
            if not hmac.compare_digest(saved_tag, expected_tag):
                raise ValueError("Wrong legacy password or corrupted package.")

        zip_bytes = self.xor_with_keystream(encrypted_data, encryption_key, nonce)
        if not zipfile.is_zipfile(io.BytesIO(zip_bytes)):
            raise ValueError("Decrypted data is not a valid project zip.")
        return zip_bytes

    def package_key_bytes(self, salt: bytes, iterations: int) -> bytes:
        if iterations > 0:
        ...
        return hashlib.sha512(PROJECT_PACKAGE_KEY + salt).digest()

    def legacy_password_key_bytes(self, password: str, salt: bytes, iterations: int) -> bytes:
        return hashlib.pbkdf2_hmac(
            "sha256",
    ...
    def package_iterations(self, package_data: bytes) -> int:
        minimum_size = len(PROJECT_PACKAGE_MAGIC) + 4
        if len(package_data) < minimum_size:
            return 0
        if package_data[:len(PROJECT_PACKAGE_MAGIC)] != PROJECT_PACKAGE_MAGIC:
            return 0
        return int.from_bytes(
            package_data[len(PROJECT_PACKAGE_MAGIC):len(PROJECT_PACKAGE_MAGIC) + 4],
            "big",
        )

    def extract_project_zip(self, zip_bytes: bytes, destination_dir: Path) -> Path:
        destination_dir.mkdir(parents=True, exist_ok=True)
        with zipfile.ZipFile(io.BytesIO(zip_bytes)) as archive:
            members = [member for member in archive.infolist() if not member.is_dir()]
            if not members:
                raise ValueError("Package zip is empty.")

            root_name = Path(members[0].filename).parts[0] if Path(members[0].filename).parts else "ImportedProject"
            project_root = self.unique_import_folder(destination_dir / root_name)
    ...
    def unique_import_folder(self, preferred_path: Path) -> Path:
        if not preferred_path.exists():
            return preferred_path

        counter = 1
        while True:
            candidate = preferred_path.with_name(f"{preferred_path.name}_{counter}")
            if not candidate.exists():
                return candidate
            counter += 1

    def find_imported_scene(self, project_root: Path) -> Optional[Path]:
        scene_path = project_root / "scene.json"
        if scene_path.exists():
            return scene_path

        scenes = sorted(project_root.glob("*.json"))
        return scenes[0] if scenes else None

    def xor_with_keystream(self, data: bytes, key: bytes, nonce: bytes) -> bytes:
    ...
    def process_thread_results(self) -> None:
        while True:
            try:
                result_type, value, error = self.thread_results.get_nowait()
            except queue.Empty:
                break

            if result_type == "export_project":
                self.finish_project_export(value, error)
            elif result_type == "import_project":
                self.finish_project_import(value, error)

    def poll_thread_results(self) -> None:
        self.process_thread_results()
        self.after(100, self.poll_thread_results)
