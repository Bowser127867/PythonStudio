...
class PythonStudioEditor(tk.Tk):
    ...
    def _build_explorer(self) -> None:
        ...
        ttk.Button(buttons, text="Cube", command=self.add_cube).pack(side=tk.LEFT, expand=True, fill=tk.X)
        ttk.Button(buttons, text="Sphere", command=self.add_sphere).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(4, 0))
        ttk.Button(buttons, text="Wedge", command=self.add_wedge).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(4, 0))
        ttk.Button(buttons, text="Pyramid", command=self.add_pyramid).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(4, 0))
        ttk.Button(buttons, text="Delete", command=self.delete_selected).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(4, 0))
    ...
    def _draw_wedge_3d(self, obj: SceneObject, selected: bool) -> None:
        ...
        self._draw_poly_wireframe(obj, local_vertices, edges, rx, ry, rz, selected, py + hy + 0.5)
    ...
    def _draw_poly_wireframe(
        ...
        self.viewport.create_text(label_x, projected_label_y, fill="#ffffff", text=obj.name)
